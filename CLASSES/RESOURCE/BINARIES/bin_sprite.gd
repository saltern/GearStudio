...
func serialize() -> PackedByteArray:
	...
	# Palette
	if clut != CLUT.NONE:
		stream.put_data(get_palette())

	# Pixel data
	match mode:
		Mode.RAW:
			stream.put_data(get_pixels())
		Mode.PALETTE:
			pass
		_:
			stream.put_data(SpriteCompression.compress_acpr(get_pixels(), bit_depth))
...
func set_palette(new_palette: PackedByteArray) -> void:
	new_palette.resize(COLOR_SIZE * get_color_count())
	palette = new_palette
