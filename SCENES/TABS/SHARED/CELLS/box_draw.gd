...
func load_boxes(boxes: Array[BoxInfo]) -> void:
	...
	for box in boxes.size():
		...
		obj_state.box_deselected_all.connect(new_box.external_deselect_all)
		obj_state.box_deselected.connect(new_box.external_deselect)
		obj_state.box_editing_toggled.connect(new_box.check_enable)
		obj_state.box_drawing_mode_changed.connect(new_box.check_enable)
		obj_state.box_multi_dragged.connect(new_box.multi_drag)
		obj_state.box_multi_drag_stopped.connect(new_box.multi_drag_stop)
...
func stop_drawing(at: Vector2, finished: bool) -> void:
	...
	SessionData.box_append(new_box)
